...
export default function RegisterPage() {
  ...
  const handleOAuthSignup = async (provider: 'google' | 'github' | 'twitter') => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to start ${provider} signup`);
      }
          ...
          <div className="bg-slate-800/90 backdrop-blur-sm rounded-2xl p-8 border border-slate-700 shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {loading === 'twitter' ? 'Redirecting...' : 'Sign up with X'}
              </button>
            </div>
            ...
            {showEmailForm ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading !== null}
                  className="w-full px-4 py-3 bg-argus-600 hover:bg-argus-500 text-white font-medium rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
