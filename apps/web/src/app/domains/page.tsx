        ...
        {domains.map((domain: any) => (
          ...
          >
            ...
            <div className="mt-4 flex items-center gap-2">
              <span className={`px-2 py-1 rounded text-xs ${domain.isBuiltIn ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                {domain.isBuiltIn ? 'Built-in' : 'Custom'}
              </span>
              <span className="text-xs text-slate-400">{domain.slug}</span>
            </div>
          </a>
        ))}
      </div>
